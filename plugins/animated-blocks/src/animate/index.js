...
registerBlockType( 'ab/animate', {
	...
	edit( { attributes, setAttributes, clientId } ) {
		...
		useEffect( () => {
			...
			// Convert seconds to milliseconds from plugin version 1.1.0
			if ( typeof duration == "number" && duration <= 10 ) {
				console.log('convert duration');
				let newDuration = String( duration * 1000 );
	    	setAttributes( { duration: newDuration } );
	    }

	    if ( typeof delay == "number" && delay <= 10 ) {
				console.log('convert delay');
				let newDelay = String( delay * 1000 );
